...
const CurrentOrder = () => {
    ...
    try {
      ...
      if (results?.data?.data) {
        ...
        localStorage.setItem(
          "step4StepResults",
          JSON.stringify(currentBooking?.step4)
        );
        context.step4StepResults = currentBooking?.step4 || [];
              ...
              <div className=" rounded-lg border m-2">
                <Collapse defaultActiveKey={["0", "1", "2"]} ghost>
                  <Panel header={key} key={i}>
                    <div className="m-2">{itemInfoFun(groupedItems[key])}</div>
                  </Panel>
                </Collapse>
              </div>
            </div>
        ...
        {Object.keys(groupedDetailsItems).map((keyName, index) => (
          <>
            <div className="p-3">
              ...
              {console.log(
                "groupedDetailsItems[keyName] is",
                groupedDetailsItems[keyName]
              )}
              {groupedDetailsItems[keyName]?.map((item, index) => {
                return (
                  <>
                    <div className="pl-5">
                      • 1 X{" "}
                      {item?.level1?.["Action 1"]
                        ? item?.level1?.["Action 1"] + " "
                        : ""}
                      {item?.level1?.["Action 2"]
                        ? item?.level1?.["Action 2"] + " "
                        : ""}
                      {item?.level1?.["Action 3"]
                        ? item?.level1?.["Action 3"] + " "
                        : ""}
                      {item?.level1?.["Action 4"]
                        ? item?.level1?.["Action 4"] + " "
                        : ""}
                    </div>
                  </>
  ...
  return (
    <>
      {console.log(
        "currentBookingDetail is " + currentBookingDetail,
        " customer is " + customer
      )}
      {isLoading ? (
        <>
          <div className="lodingScreen">
            <Spin size="large" />
          </div>
        </>
      ) : (
            ...
            <div className="step7RespoMain">
              <div className="step7RespoContainer1">
                    ...
                    <div className="flex flex-row justify-between p-4 rounded-xl">
                      <div className="step7_grid2 justify-start">
                        ...
                        <div className="flex flex-col my-auto px-2 pr-2">
                          <div className=" font-bold">
                            {moment(currentBookingDetail.shiftingOn).format(
                              "dddd"
                            )}
                          </div>
                          <div>
                      ...
                      <div className="flex flex-col gap-3 text-md">
                        ...
                        <div className="OrderID_text_step7 font-semibold">
                          {currentBookingDetail.booking_id}
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex flex-row justify-between p-3">
                        <div className="step7Summarybox_item1">
                          ...
                          <div className="font-semibold">
                            {currentBookingDetail.shiftingFrom}
                          </div>
                        </div>
                        <div>
                          <div>To</div>
                          <div className="font-semibold">
                            {currentBookingDetail.shiftingTo}
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-row justify-between p-3">
                        <div>
                          ...
                          <div className="font-semibold">
                            {currentBookingDetail.moverPlanner
                              ? currentBookingDetail.moverPlanner
                              : "SUMIT"}
                          </div>
                        ...
                        <div className="step7Summarybox_item2">
                          ...
                          <div className="font-semibold">
                            {currentBookingDetail.moverManager
                              ? currentBookingDetail.moverManager
                              : "Not Assigned"}
                          </div>
                        </div>
                      ...
                      <div className="flex flex-row p-3 justify-center">
                        ...
                        <div>
                          {/* <button
                            ...
                            type="submit"
                            onClick={(e) =>
                              handleEditInventory(e, currentBookingDetail)
                            }
                          >
                            <img
                ...
                <div className="step7ProfileBox1 m-5 rounded-lg bg-white pb-1 step7_2container">
                  ...
                  <hr className="mx-auto step7SummarylBox2_hr" />
                  <div className="p-3">Your order is being evaluated by us</div>
                  <div className="p-3">
                    <Timeline>
                      <Timeline.Item>
                        ...
                        <div className="py-1 font-semibold">
                          {currentBookingDetail.currentFloor} Floor
                        </div>{" "}
                        <div className="py-1 greencolor">
                          {currentBookingDetail.isLiftAvailableOnCurrentFloor
                            ? "Lift Available"
                            : "Lift Not Available"}{" "}
                        </div>
                      </Timeline.Item>
                      <Timeline.Item>
                        ...
                        <div className="py-1 font-semibold">
                          {currentBookingDetail.movingOnFloor} Floor
                        </div>{" "}
                        <div className="py-1 greencolor">
                          {currentBookingDetail.isLiftAvailableOnMovingFloor
                            ? "Lift Available"
                            : "Lift Not Available"}
                        </div>
                      </Timeline.Item>
                  ...
                  <div className="flex flex-row justify-between p-4 mt-6">
                    <div>
                      ...
                      <div className="font-semibold">
                        {currentBookingDetail.shiftingFor}
                      </div>
                    </div>
                  </div>
