...
const Step9 =() =>{
    ...
    return (
      <div className="flex group flex-nowrap flex-row space-x-2" onClick={handleClick}>
        <div className="flex-col w-1/5 mt-4 ">
          {washingmachine.map(element => (
         <Card image={"images/washing-machine-24.png"} item={element} itemCount={1} />
        ))}
        </div>
  
        <div className={`w-1/5 ${display}  px-2 py-4` }>
          {washingmachineModel.map(element =>(
            <Card image={""} item={element} />
          ))}
        </div>
   
        <div className={`w-1/5 ${display}  px-2 py-4` }>
          {washingmachineType.map(element =>(
            <Card image={""} item={element} />
          ))}
        </div>
      </div>
